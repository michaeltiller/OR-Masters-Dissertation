...
def get_subplot_grid(n_plots, max_cols=2):
...
def plotDRUsage(L_net_vals, delta_L_vals, delta_L_Flex_vals, 
    ...
    for idx, s in enumerate(S):
        ...
        ax.set_xlabel("Time")
        ax.set_ylabel("DR Contribution (MW)")
...
def plotLoadprofiles(L_net_vals, delta_L_vals, delta_L_Flex_vals,
    ...
    for idx,s in enumerate(S):
        ...
        ax.plot(
            ...
            color='#1565C0',
            lw=2,
            label="Base load + DR Contribution"
        )
    ...
    fig2.suptitle(
        f"Base Load vs Load with DR Contribution by Scenario, "
        f"Objval:{round(objval,2)}, "
        f"Ramping:{round(ramping,2)}, "
        f"Peak:{round(peak,2)}",
        fontsize=13
