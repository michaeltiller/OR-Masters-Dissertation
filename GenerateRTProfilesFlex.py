...
def generateRTProfiles(
    ...
    settle_tol=0.02,
    noise_seed=None,
    interrupt_lengths=None,  # dict {resource: [L1, L2, ...]} in hours, fractional OK (e.g. 0.5); resource absent -> single "full" variant
):
    INTERVALS_PER_HOUR = 12
    HOURS_PER_DAY = 24
    ...
    def upsample_physical(profile, tau, duration_hours=None):
        ...
        If duration_hours is given, the negative (interruption) portion of
        the 5-min setpoint is zeroed out after that many hours from its
        start, measured in 5-min intervals so fractional hours (e.g. 0.5)
        work — the raw hourly profile itself stays untouched.
    ...
    # resource -> list of duration labels (hours, possibly fractional), or
    # ["full"] when no interrupt_lengths given for that resource
    raw_by_resource_duration = {}
    for r in delta_raw:
        lengths = interrupt_lengths.get(r)
        raw_by_resource_duration[r] = list(lengths) if lengths else ["full"]

    # 5-min profile per (resource, duration_label), used for both the plot
    # grid and the enumeration loop below
    delta_5min = {}
    for r, dlabels in raw_by_resource_duration.items():
        for dlabel in dlabels:
            duration_hours = None if dlabel == "full" else dlabel
            y, noise_mask = upsample_physical(delta_raw[r], tau_minutes[r], duration_hours=duration_hours)
            std = noise_std.get(r, 0.0) * np.max(np.abs(delta_raw[r]))
            if std > 0:
                noise = ar1_noise(len(y), std, noise_phi)
                y = y + noise * noise_mask
            delta_5min[(r, dlabel)] = y

    resource_names = list(delta_raw.keys())
    L = range(len(resource_names))
    raw_profiles = {l: [] for l in L}
    raw_starts = {l: [] for l in L}
    raw_durations = {l: [] for l in L}  # interrupt-length label per i, parallel to raw_starts
    ...
    if showPlot:
        ...
        for i, ((resource, dlabel), profile) in enumerate(delta_5min.items()):
            ax = axes[i]
            ...
            label = resource if dlabel == "full" else f"{resource} ({dlabel}h)"
            ax.set_title(f"RT DR Resource: {label}", fontsize=10)
    ...
    for l, resource in enumerate(resource_names):
        for dlabel in raw_by_resource_duration[resource]:
            profile = delta_5min[(resource, dlabel)]
    ...
    for l in L:
        ...
        duration_of[l] = raw_durations[l] + [None] * pad
    ...
    return {
        ...
        "valid_i": valid_i,
        "start_of": start_of,
        "duration_of": duration_of,   # interrupt length (hours, fractional OK) per (l,i)
